...
workpar_sum <- work_pov_zcta.2 %>% 
  summarise(workpar_pov = sum(workpar_pov),
            workpar = sum(workpar))
#1,412,546 children (0-5) with working parents in Texas (1,393,381 last year)
#543,137 of low income children (0-5) with working parents in Texas (555,324 last year)
...
board.targets <- read_xlsx(here("Childcare","Board Targets 2022.xlsx")) #Extract daily targets

subsidy_enrollment <- read_xlsx(here("Childcare", "Children_at_Risk_ORR_R011062-110221__-_Sep21 (3).xlsx")) 

providers <- read_csv(here("Childcare", "ChildCareSearchResults (2).csv")) %>% 
  filter(Type!="Listed Family Home" & Type!="Small Employer Based Child Care")

#add subsidy and trs info:
providers_detail <- read_xlsx(here("Childcare", "Children_at_Risk_ORR_R011062-110221__-_Sep21 (3).xlsx")) %>% 
...
write_csv(providers, here("geocode for matching", "providers.to.geocode.csv"))
